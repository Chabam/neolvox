#ifndef DEF_CT_ABSTRACTGROUPMODELOUT_H
#define DEF_CT_ABSTRACTGROUPMODELOUT_H

#include "ct_model/abstract/ct_abstractgroupmodelt.h"
#include "ct_model/outModel/abstract/ct_outabstractsingularitemmodel.h"
#include "ct_model/outModel/abstract/ct_outabstractgroupmodel.h"

typedef CT_AbstractGroupModelT<CT_OutAbstractSingularItemModel, CT_OutAbstractGroupModel> DEF_CT_AbstractGroupModelOut;

#endif // DEF_CT_ABSTRACTGROUPMODELOUT_H
