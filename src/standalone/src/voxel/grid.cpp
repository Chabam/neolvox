...
Grid::Grid(const Bounds& bounds, double cell_size, bool compute_variance)
    ...
    , m_hits{cell_count(), std::allocator<std::atomic<size_t>>{}}
    , m_counts{cell_count(), std::allocator<std::atomic<size_t>>{}}
    , m_lengths{cell_count(), std::allocator<std::atomic<double>>{}}
    , m_lengths_variances{std::invoke([this, compute_variance]() -> decltype(m_lengths_variances) {
        if (compute_variance)
            return std::vector<std::atomic<double>>{
                cell_count(), std::allocator<std::atomic<double>>{}
            };
        return {};
    })}
...
Grid::Grid(Grid&& other)
    ...
    , m_lengths_variances{std::move(other.m_lengths_variances)}
