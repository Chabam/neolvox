...
class VoxelsMetrics
{
  ...
  private:
    Grid m_grid;
    std::map<Index, Index> m_chunk_mapping;
    ...
    [[nodiscard]]
    auto insert_if_not_found(Index idx) -> std::pair<size_t, size_t>;
    [[nodiscard]]
    auto insert_if_not_found(const Index3D& idx) -> std::pair<size_t, size_t>;

    template <typename PadFunc>
    auto compute_pad_impl(PadFunc&& func) -> void
    {
        const auto chunk_count = m_coords.size();
        m_pad = chunked_vector<double>{chunk_count};
