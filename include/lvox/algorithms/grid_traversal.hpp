...
template <typename GridT, bool exact_distance>
struct GridTraversal
{
    GridTraversal(const GridT& grid, pad_compute_methods::PadComputeMethod& pcm)
        : m_grid{grid},
          m_pad_compute_method{pcm}
    {}
    ...
    ) const -> void
    {
        ...
        do
        {
            ...
            if (current_hit.m_distance_in_voxel > 0)
                std::visit(ComputeGridCountsAndLengths{current_hit}, m_pad_compute_method);

            current_hit.m_index = Index3D{current_voxel_x, current_voxel_y, current_voxel_z};
        } while (can_continue);
    }
    const GridT& m_grid;
    pad_compute_methods::PadComputeMethod& m_pad_compute_method;
};

template <typename GridT>
using GridTraversalVoxelRounding = GridTraversal<GridT, false>;

template <typename GridT>
using GridTraversalExactDistance = GridTraversal<GridT, true>;
