...
- Unmanned laser scanner (ULS)

The files it can read are all the ones supported by [[https://pdal.io/en/stable/stages/readers.html][PDAL]].

*** TLS

neolvox works out of the box with multiple scans of the same scene. To
register the scene and its various scan location, you must use the
=-s= and =-o= (=--origin=) flags respectively:

#+begin_src shell

lvox -s /path/to/a/point_cloud_file_0.laz -o  0  0  0 \ # The first scan at (0,0,0)
     -s /path/to/a/point_cloud_file_1.laz -o  0  5  0 \ # The second scan at (0,5,0)
     -s /path/to/a/point_cloud_file_2.laz -o  0 -5  0 \ # and so on...
     -s /path/to/a/point_cloud_file_3.laz -o  5  0  0 \
     -s /path/to/a/point_cloud_file_4.laz -o -5  0  0
...
For MLS, in addition of a point cloud file (specified with the =-s=
flag, same as with TLS), the user must provide another point cloud
file for the trajectory taken by the scanner. This can be done with
the =-t= or =--trajectory= flag. The trajectory must contain points
that have GPS time recorded.
...
root
  \_ lvox
     |- pad
     |- counts (only with the `-a' flag)
     |- hits (only with the `-a' flag)
     |- hits lenghts (only with the `-a' flag)
     |- x
     |- y
     \_ z
#+end_src
...
- *Minimal coordinates values*: the minimal coordinates as a =(x,y,z)=
  triplet.

- *Maximum coordinates values*: the maximum coordinates as a =(x,y,z)=
  triplet.

- *Minimum index values*: the minimum index of the grid as a =(x,y,z)=
  triplet.

- *Maximum index values*: the maximum index of the grid as a =(x,y,z)=
  triplet.

- *Missing hits value*: the value used to represent PAD values with
  not enough hits to be considered in the PAD estimation
...
*** Building
Building neolvox is done through =CMake=. To generate a build script
for your platform, you can use a =CMakePreset=:
...
- [X] Support mutliple scans as arguments in the standalone version
- [X] Bounding box as input
- [X] Bounding aligned to axis at (0,0,0)
- [X] Handling classes
